...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        if (vma && ((perm & vma->vma_prot) == perm)) {
               
                dbg(DBG_PRINT,"(GRADING3B 1)\n"); 
                return 1;
                
                //dbg(DBG_TEST, "ummaped addr -> 0x%p, perm -> %d, vam_prot -> %d\n", vaddr, perm,vma->vma_prot);
                // dbg(DBG_VMMAP,"(GRADING3B)\n");
                // return 0;
        }
        else {
                //dbg(DBG_TEST, "ummaped addr -> 0x%p\n, perm -> %d", vaddr, perm);
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uintptr_t cur = (uintptr_t) avaddr;
                dbg(DBG_PRINT,"(GRADING3B 1)\n");
        while (cur < (uintptr_t)avaddr + len) {
                if (addr_perm(p, (void *)cur, perm) == 0) {
                                dbg(DBG_PRINT,"(GRADING3D 1)\n");
                                return 0;
                }
                cur += PAGE_SIZE;
