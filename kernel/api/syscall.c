...
sys_read(read_args_t *arg)
{
        ...
        copy_from_user(&read_args, arg, sizeof(read_args));
        ...
        //         return -1;
        // }
        int bytes = do_read(read_args.fd, temp_buf, read_args.nbytes);
        dbg(DBG_PRINT,"(GRADING3C 1)\n");
        if (bytes < 0) {
                page_free(temp_buf);
        ...
        copy_to_user(arg->buf, temp_buf, bytes);
        dbg(DBG_PRINT,"(GRADING3C 1)\n");
        // if (retval < 0) {
        //         //        page_free(temp_buf);
        //         curthr->kt_errno = -retval;
...
sys_write(write_args_t *arg)
{
        ...
        copy_from_user(&write_args, arg, sizeof(write_args));
        ...
        //         return -1;
        // }
        copy_from_user(temp_buf, write_args.buf, write_args.nbytes);
        dbg(DBG_PRINT,"(GRADING3B 1)\n");
        // if (retval < 0) {
        //        curthr->kt_errno = -retval;
        //        dbg(DBG_VMMAP,"(GRADING3B)\n");
        //        return -1;
        // }
        int bytes = do_write(write_args.fd, temp_buf, write_args.nbytes);
        dbg(DBG_PRINT,"(GRADING3B 1)\n");
        if (bytes < 0) {
                page_free(temp_buf);
                curthr->kt_errno = -bytes;
...
sys_getdents(getdents_args_t *arg)
{
       ...
       //int retval;
       copy_from_user(&getdents_args, arg, sizeof(getdents_args));
...
//               return -1;
//        }
       // allocate a singel dirent_t
       dirent_t dirent;
       uint32_t cnt;
       int total = 0;
       dbg(DBG_PRINT,"(GRADING3B 6)\n");
       // need to loop a max of something like count / sizeof(dirent_t) times.
       for (cnt = 0; cnt < getdents_args.count / sizeof(dirent_t); cnt++) {
               // call do_getdent in a loop until you have read getdent_args_t->count bytes (or an error occurs)
               int bytes = do_getdent(getdents_args.fd, &dirent);
               dbg(DBG_PRINT,"(GRADING3B 6)\n");
               if (bytes < 0) {
                       curthr->kt_errno = -bytes;
                       dbg(DBG_PRINT,"(GRADING3D 1)\n");
                       ...
                       copy_to_user(getdents_args.dirp + cnt, &dirent, bytes);
                       dbg(DBG_PRINT,"(GRADING3B 6)\n");
                //        if (retval < 0) {
                //                curthr->kt_errno = -retval;
                //                dbg(DBG_VMMAP,"(GRADING3B)\n");
...
static pid_t sys_waitpid(waitpid_args_t *args)
{
        ...
        if (0 > copy_from_user(&kargs, args, sizeof(kargs))) {
                //dbg(DBG_TEST,"____________________________copy from user\n");
                curthr->kt_errno = EFAULT;
                return -1;
        }
