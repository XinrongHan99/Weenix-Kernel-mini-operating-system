...
sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        if (curthr->kt_cancelled == 1) {  // if it is canceled
                ////dbg(DBG_PRINT, "(GRADING1C)\n");
                return -EINTR;
        } else {  // if not
                // 1. update thread states
                curthr->kt_state = KT_SLEEP_CANCELLABLE;
                // 2. enqueue it to the given queue
                ktqueue_enqueue(q, curthr);
                sched_switch();// curthr sleep and another curthr in different context run
                if (curthr->kt_cancelled == 1) {  // if it is canceled during sleep
                        ////dbg(DBG_PRINT, "(GRADING1C)\n");
                        return -EINTR;
                }
                ////dbg(DBG_PRINT, "(GRADING1C)\n");
...
sched_cancel(struct kthread *kthr)
{
        ...
        //unless the thread is in the KT_NO_STATE or KT_EXITED
        //state, it should be on some queue
        if(kthr->kt_state == KT_NO_STATE && kthr->kt_state == KT_EXITED){
                ////dbg(DBG_PRINT, "(GRADING1C)\n");
                return;
        }else if(kthr->kt_state == KT_SLEEP_CANCELLABLE){
                //remove the thread from the queue it is sleeping on
...
sched_switch(void)
{
        ...
        //1. masking interrupts: set IPL to high
        intr_setipl(IPL_HIGH);
        while(sched_queue_empty(&kt_runq)){
                intr_setipl(IPL_LOW);
                intr_wait(); //TODO: how to use intr_wait() 
                intr_setipl(IPL_HIGH);
                ////dbg(DBG_PRINT, "(GRADING1A)\n");
        }
        ////dbg(DBG_PRINT, "(GRADING1C)\n");
