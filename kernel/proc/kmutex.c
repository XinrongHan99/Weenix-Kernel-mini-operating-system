...
//        //dbg(DBG_PRINT, "(GRADING1A 6.a)\n");
       // if the mutex is already taken
       if (mtx->km_holder != NULL) {
                sched_sleep_on(&mtx->km_waitq);
                // //dbg(DBG_PRINT, "(GRADING1C)\n");
       }
       else { // mutex not taken, then current thread take the mutex
...
//        //dbg(DBG_PRINT, "(GRADING1C)\n");
       ...
       if (mtx->km_holder != NULL) {
               ...
               if(mtx->km_holder == curthr && retval == -EINTR){
                        kmutex_unlock(mtx);
                        // //dbg(DBG_PRINT, "(GRADING1C)\n");
...
//        //dbg(DBG_PRINT, "(GRADING1A 6.c)\n");
       // if there are threads waiting in queue for mutex
       if (mtx->km_waitq.tq_size != 0) {
               mtx->km_holder = sched_wakeup_on(&mtx->km_waitq);
        //        //dbg(DBG_PRINT, "(GRADING1C)\n");
       }
       else {
       // no thread waiting, give up mutex
               mtx->km_holder = NULL;
        //        //dbg(DBG_PRINT, "(GRADING1C)\n");
       }
