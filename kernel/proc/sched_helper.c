...
sched_wakeup_on(ktqueue_t *q)
{
        ...
        if(!sched_queue_empty(q)){
                //return some thread(which one should I return?)
                kthread_t* dthr = ktqueue_dequeue(q);

                KASSERT((dthr->kt_state == KT_SLEEP) || (dthr->kt_state == KT_SLEEP_CANCELLABLE));
...
sched_broadcast_on(ktqueue_t *q)
{
        ...
        KASSERT(q != NULL);
        // list_iterate_begin(&q->tq_list,thr,kthread_t,kt_qlink){
        //         list_remove(&thr->kt_qlink)
        //         //put them into the runqueue
        // }
        // list_iterate_end();
        kthread_t *thr;
