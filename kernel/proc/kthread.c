...
kthread_exit(void *retval)
{
       ...
       KASSERT(!curthr->kt_wchan); // current thread not in any queue
...
kthread_clone(kthread_t *thr)
{
       ...
       new_thread->kt_kstack = alloc_stack();
       context_setup(&new_thread->kt_ctx, NULL, 0, NULL, new_thread->kt_kstack, DEFAULT_STACK_SIZE, thr->kt_proc->p_pagedir);
