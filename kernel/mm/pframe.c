...
pframe_alloc(mmobj_t *o, uint32_t pagenum)
{
        ...
        // dbg(DBG_PFRAME,"alloc #%d, pin num = %d\n\n", pf->pf_pagenum, pf->pf_pincount);
        return pf;
}
...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        //dbg(DBG_TEST,"start pagenum = %d\n", pagenum);
        pframe_t *pf;
        // determine if the page is already resient in memory -> use pframe_get_resident
        pf = pframe_get_resident(o, pagenum);
        dbg(DBG_PRINT,"(GRADING3A)\n");
        if (pf) {
                // the page is found (resident) but busy
                while (pframe_is_busy(pf)) { // while loop since the page might not still be resident after we finish waiting
                        // wait for it to become unbusy and then try again
                        // 也就是说一个进程call了pframe_get但是这个pframe正在被其他进程使用，那么就在pframe的pf_waitq上面等
                ...
                *result = pf;
                ...
                // the page is not found, allocte a new page and fill it
                // Before allocating the new pframe, we check to see if we need to call pageoutd and wake it up if necessary.
                if (pageoutd_needed()) {
                        pageoutd_wakeup();
                ...
                dbg(DBG_PRINT,"(GRADING3A)\n");
                // if it fails to fill the page. we should not return the incomplete page to the caller
                if (ret < 0) {
                        pframe_free(pf);
                        //dbg(DBG_TEST,"%p return: %d\n",*result,ret);
                        dbg(DBG_PRINT,"(GRADING3A)\n");
                        return ret;
                }
                *result = pf;
...
pframe_pin(pframe_t *pf)
{
        ...
        //dbg(DBG_PFRAME,"\tpin #%d, pin num = %d -> %d\n", pf->pf_pagenum, pf->pf_pincount,pf->pf_pincount+1);
        ...
        dbg(DBG_PRINT,"(GRADING3A 1.b)\n");
        // If the pframe has not yet been pinned -> pframe_is_pinned return the pin count of the pframe_t
        
        pf->pf_pincount++;
        dbg(DBG_PRINT,"(GRADING3A)\n");
        
        if (pf->pf_pincount == 1) {
                // remove this pframe's list link from the allocated list
                list_remove(&pf->pf_link);
                nallocated--;
                ...
                npinned++;
                dbg(DBG_PRINT,"(GRADING3A)\n");
                //dbg(DBG_PFRAME,"     +++++++ #%d, npinned = %d\n", pf->pf_pagenum, npinned);
        }
        dbg(DBG_PRINT,"(GRADING3A)\n");
}
...
pframe_unpin(pframe_t *pf)
{
        ...
        //dbg(DBG_PFRAME,"unpin #%d, pin num = %d -> %d\n", pf->pf_pagenum, pf->pf_pincount,pf->pf_pincount-1);
        KASSERT(!pframe_is_free(pf)); /* can only pin a pframe object that's "in-use" */
