...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        // If the file is a block device, return -ENOTSUP
        ...
        // Check if the file is a byte device
        if (S_ISCHR(file->vn_mode)) {
                // Find the file's bytedev_t
                bytedev_t *byte_device = bytedev_lookup(file->vn_devid);
                if (!byte_device) {
                        //dbg(DBG_PRINT, "(GRADING2A)\n");
                        return -ENODEV;
                }

                // Call read on the byte device
        ...
        //dbg(DBG_PRINT, "(GRADING2A)\n");
        // If the file is not a byte or block device, return an appropriate error
        return res;
        ...
        // int read_res = S_ISCHR(file->vn_mode) ? file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count) : -ENOTSUP;        
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        int write_res = S_ISCHR(file->vn_mode) ? file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count) : -ENOTSUP;
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
        ...
        // {
                ...
                // Call mmap on the byte device
                int res = file->vn_cdev->cd_ops->mmap(file, vma, ret);
                // dbg(DBG_TEST,"res is: %d\n",res);
                dbg(DBG_PRINT,"(GRADING3C 1)\n");
                return res;
        // }
//         dbg(DBG_VMMAP,"(GRADING3B)\n");
